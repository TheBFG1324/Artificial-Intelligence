...
def is_terminal(state):
    ...
    if state[0]+state[6]+state[10]+state[15]=='XXXX' or state[0]+state[6]+state[10]+state[15]=='OOOO': return True
...
def new_node(state):
    global states_examined, seen_set, seen_list, transposition_table
    states_examined += 1
    seen_set.add(state)
...
def max_value(state):
    global terminals_found, transposition_table
...
def alpha_beta_searchRN(state, depth=0, alpha=float('-inf'), beta=float('inf')):
    global terminals_found
    if is_terminal(state):
        terminals_found += 1
        return utility(state)
...
def max_value(state, depth, alpha, beta):
    ...
    if state in transposition_table:
        stored_depth, stored_value = transposition_table[state]
    ...
    if is_terminal(state):
        terminals_found += 1
        value = utility(state)
        transposition_table[state] = (depth, value)
        return value
    ...
    for move in get_available_moves(state):
        new_state = next_state(state, move)
        new_node(new_state)
        v = max(v, min_value(new_state, depth + 1, alpha, beta))
        if v >= beta:
            break  # Beta cut-off
        alpha = max(alpha, v)

    transposition_table[state] = (depth, v)  # Store the value with depth
    return v
...
def min_value(state, depth, alpha, beta):
    ...
    for move in get_available_moves(state):
        new_state = next_state(state, move)
        new_node(new_state)
        v = min(v, max_value(new_state, depth + 1, alpha, beta))
        if v <= alpha:
            break  # Alpha cut-off
        beta = min(beta, v)
